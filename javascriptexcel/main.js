...
workbook.xlsx.readFile(filename).then(()=>{
    ...
    for(var i = 2; i <= worksheet.rowCount; i++) {
        ...
        var sicilno = yil.charAt(3) + rol + rolsayi + 'T' + ((i-1)-tekrarlisayi);
        var kartid;
        ...
        sql4=`SELECT count(*) FROM users where tcno=? AND isim_soyisim=? AND rol=?`;
        vt.all(sql4, [tckn, adsoyad, rol_ham], (err, rows) => {
            if(rows[0]['count(*)'] != 0) {
                console.log("Kişi zaten veritabanına kayıtlı.");
            }else {
                sql2=`SELECT count(*) FROM users where tcno=?`;
                vt.all(sql2, [tckn], (err, rows) => {
                    if(err) return console.error(err.message);
                    console.log(rows[0]['count(*)'] );
                    if(rows[0]['count(*)'] != 0) {
                        //console.log("Veritabanına " + tckn + " tcknli " + adsoyad + " adlı kişi yazılamadı.");
                        sql3 =`UPDATE users SET silindimi=? where tcno=?`;
                        vt.run(sql3,["true", tckn], (err)=> {
                            if(err) return console.error(err.message);
                        })
                    }//else {
                        sql = `INSERT INTO users (tcno,isim_soyisim,rol,sicilno,kartid,silindimi) VALUES (?,?,?,?,?,"false")`;
